...
Enemy::Enemy(ProjectileManager* projMan, EntityManager* entityMan, SpriteSheetInfo bar, float _x, float _y, int _i, entitytype entType, int _index)
{
	...
	else if (entityType == BOSS)
	{
		...
		texture->setSpriteFrameSize(w, h);
		//pewpewing
		texture->addSpriteAnimRow(4, 0, 1, w + 1, 0, 8);
		texture->addSpriteAnimRow(5, 0, h+2, w + 1, 0, 8);
		texture->addSpriteAnimRow(6, 0, 2*h+3, w + 1, 0, 8);
		texture->addSpriteAnimRow(7, 0, 3*h+4, w + 1, 0, 8);
		//walking
		texture->addSpriteAnimRow(0, 0, 4*h+5, w + 1, 0, 8);
		texture->addSpriteAnimRow(1, 0, 5*h+6, w + 1, 0, 8);
		texture->addSpriteAnimRow(2, 0, 6*h+7, w + 1, 0, 8);
		texture->addSpriteAnimRow(3, 0, 7*h+8, w + 1, 0, 8);
	}
	else
	{
		hp = 350;
